...
import {
  COMMAND_PREFIX,
  PIXEL_RACERS_HONORARIES_DISCORD_TOKEN,
  GUILD_ID,
  CHANNEL_ID,
  NETWORK,
  PIXEL_RACERS_HONORARIES_ID,
...
const initBot = async () => {
  ...
  signatures = await connection.getSignaturesForAddress(new PublicKey(PIXEL_RACERS_HONORARIES_ID), {});
  if (signatures[0].signature) options.until = signatures[0].signature

  while (true) {
    try {
      signatures = await connection.getSignaturesForAddress(new PublicKey(PIXEL_RACERS_HONORARIES_ID), options);
...
try {
  ...
  client.login(PIXEL_RACERS_HONORARIES_DISCORD_TOKEN);
} catch (e) {
